...
static void leds__get_index_position(uint8_t index, uint8_t *position);
...
static void leds__get_index_position(uint8_t index, uint8_t *position) {
    position[0] = index / LED_COLS;
    position[1] = index % LED_COLS;
}
            ...
            switch (event.type) {
                case EVENT_BT_ADV:
                    if (bt_effect_enabled) {
                        ...
                        leds__get_index_position(event.data, position);
                        new_effect.type = SLOW_PULSE;
                        new_effect.row_gpio = row_gpios[position[0]];
                        new_effect.col_channel = col_channels[position[1]];
                    }
                    break;
